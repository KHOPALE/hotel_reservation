...
class HotalReservation():
    ...
    def get_predicted_res(self): # Public Method
        ...
        # test_array[0][0] = self.no_of_adults['no_of_adults']
        # test_array[0][1] = self.no_of_children['no_of_children'][self.no_of_children]
        # test_array[0][2] = self.no_of_weekend_nights['no_of_weekend_nights'][self.no_of_weekend_nights]
        # test_array[0][3] = self.required_car_parking_space['required_car_parking_space'][self.required_car_parking_space]
        # test_array[0][4] = self.room_type_reserved['room_type_reserved'][self.room_type_reserved]
        # test_array[0][5] = self.market_segment_type['market_segment_type'][self.market_segment_type]
        # test_array[0][6] = self.no_of_previous_cancellations['no_of_previous_cancellations']
        # test_array[0][7] = self.no_of_special_requests['no_of_special_requests'][self.no_of_special_requests]
        # # test_array[0][8] = self.type_of_meal_plan_Meal['type_of_meal_plan_Meal'][self.type_of_meal_plan_Meal]
        # type_of_meal_plan_Meal= 'type_of_meal_plan_Meal_' + self.type_of_meal_plan_Meal
        # index = self.project_data['Column Names'].index(type_of_meal_plan_Meal)
        
        # test_array[0][9] =self.no_of_week_nights['no_of_week_nights'][self.no_of_week_nights]
        # test_array[0][10] =self.lead_time['lead_time'][self.lead_time]                       
        # test_array[0][11] =self.arrival_year
        # test_array[0][12]=self.arrival_month                    
        # test_array[0][13] =self.arrival_date
        # test_array[0][14]=self.repeated_guest['lead_time'][self.lead_time]
        # test_array[0][15]=self.avg_price_per_room['avg_price_per_room'][self.avg_price_per_room]
        # test_array[0][16]=self.no_of_previous_bookings_not_canceled['no_of_previous_bookings_not_canceled'][self.no_of_previous_bookings_not_canceled] 
